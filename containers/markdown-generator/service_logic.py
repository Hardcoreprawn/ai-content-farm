...
class MarkdownGenerator:
    ...
    async def generate_markdown_from_ranked_content(
        ...
        try:
            if not content_items:
                raise ValueError(
                    "No content items provided for markdown generation")
    ...
    def _generate_post_markdown(
        ...
        return self._generate_standard_markdown(
            clean_title, slug, ai_summary, topics, sentiment,
            final_score, engagement_score, site_name, source_url,
            content_type, published_at, rank
        )

    def _generate_standard_markdown(
        self, clean_title: str, slug: str, ai_summary: str, topics: List[str],
        sentiment: str, final_score: float, engagement_score: float,
        site_name: str, source_url: str, content_type: str,
        published_at: str, rank: int
    ) -> str:
...
*Content curated and generated by AI Content Farm*"""
    ...
    async def _save_generated_markdown_to_blobs(
        ...
        try:
            ...
            manifest['storage_location'] = f"blob://{config.GENERATED_CONTENT_CONTAINER}/markdown/{timestamp}/"
...
class ContentWatcher:
    ...
    async def check_for_new_ranked_content(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Generate markdown
            generation_result = await self.generator.generate_markdown_from_ranked_content(
                content_items
            )
    ...
    async def _get_latest_ranked_content(self) -> Optional[Tuple[List[Dict[str, Any]], str]]:
        """
