...
with patch('main.BlobStorageClient', return_value=mock_blob_client), \
        patch('main.MarkdownGenerator', return_value=mock_generator), \
        patch('main.ContentWatcher', return_value=mock_watcher), \
        patch('main.HealthChecker', return_value=mock_health_checker), \
        patch('main.start_content_watcher', new_callable=AsyncMock), \
        patch('config.config.validate_required_settings'):
...
class TestMainApplication:
    ...
    def test_status_endpoint(self, mock_health):
        ...
        mock_health.get_service_status = AsyncMock(return_value={
            "service": "markdown-generator",
            "status": "running",
            "timestamp": datetime.now(timezone.utc).isoformat(),
    ...
    def test_generate_markdown_success(self, mock_gen):
        ...
        mock_gen.generate_markdown_from_ranked_content = AsyncMock(return_value={
            "status": "success",
            "files_generated": 5,
    ...
    def test_trigger_check_with_new_content(self, mock_watcher):
        ...
        mock_watcher.check_for_new_ranked_content = AsyncMock(return_value={
            "status": "success",
            "files_generated": 3
        })
