...
from src.libs.standard_endpoints import create_standard_health_endpoint
...
async def check_dependencies() -> bool:
    """Check service dependencies - return boolean for health status."""
    try:
...
# Create standard health endpoint using shared library
health_endpoint = create_standard_health_endpoint(
    service_name=settings.service_name,
    version=settings.service_version,
    dependency_checks={"dependencies": check_dependencies},
...
async def root(
    ...
    Follows standard response format from libs/shared_models.py.
    """
    try:
...
async def health():
    ...
    try:
        # Simple health check that matches test expectations
...
async def status() -> StandardResponse:
    ...
    Provides comprehensive service information including configuration and dependencies.
    """
    try:
        dependencies = await check_dependencies()
...
# Processing endpoint placeholder for test requirements
