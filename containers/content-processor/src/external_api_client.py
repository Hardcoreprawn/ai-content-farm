    ...
    async def _make_openai_request(
        ...
        try:
            ...
            if response.status_code == 429:
                self.logger.warning(
                    f"Rate limited by {endpoint['region']}: {response.headers}"
                )
            ...
            self.logger.warning(f"Connection error to {endpoint['region']}: {e}")
            raise
            ...
            self.logger.error(f"Unexpected error with {endpoint['region']}: {e}")
        ...
        **kwargs,
        ...
        for attempt, endpoint in enumerate(self.openai_endpoints):
            try:
                ...
                last_error = e
                self.logger.warning(f"Failed with {endpoint['region']}: {e}")
        ...
        # All regions failed
        raise OpenAIAPIError(f"All OpenAI regions failed. Last error: {last_error}")
