"""
Phase 2B Integration Tests: Content Collector → Processor Pipeline

Tests that validate the end-to-end pipeline from content collection to processing:
- Content collector saves to COLLECTED_CONTENT container
- Content processor finds and processes unprocessed collections
- Processed content is saved to PROCESSED_CONTENT container
...
class TestPhase2BPipeline:
    """Integration tests for Phase 2B Collector → Processor Pipeline."""
    ...
    async def test_end_to_end_pipeline_simulation(self, client, blob_client):
        ...
        print(f"Debug - Direct processing result: {direct_result}")

        # Verify the direct processing worked
