...
class Config:
    ...
    STORAGE_ACCOUNT_URL = os.getenv(
        "STORAGE_ACCOUNT_URL", f"https://{os.getenv('STORAGE_ACCOUNT_NAME', 'aicontentfarm76ko2h')}.blob.core.windows.net")
    RANKED_CONTENT_CONTAINER = os.getenv(
    ...
    ENABLE_SOURCE_VERIFICATION = os.getenv(
        "ENABLE_SOURCE_VERIFICATION", "true").lower() == "true"
    VERIFICATION_TIMEOUT = int(
        os.getenv("VERIFICATION_TIMEOUT", 30))  # seconds

    # Generation Limits
    MAX_CONCURRENT_GENERATIONS = int(
        os.getenv("MAX_CONCURRENT_GENERATIONS", 3))
    MAX_RETRIES = int(os.getenv("MAX_RETRIES", 3))
    RETRY_DELAY = int(os.getenv("RETRY_DELAY", 5))

    # Quality Control
    MIN_QUALITY_SCORE = float(os.getenv("MIN_QUALITY_SCORE", 0.7))
    ENABLE_FACT_CHECK = os.getenv(
        "ENABLE_FACT_CHECK", "true").lower() == "true"
    ...
    def validate_config(cls) -> Dict[str, Any]:
        ...
        has_azure_ai = bool(
            cls.AZURE_OPENAI_ENDPOINT and cls.AZURE_OPENAI_API_KEY)
        ...
        if not (has_azure_ai or has_openai_direct or has_claude):
            issues.append(
                "No AI services configured (Azure OpenAI, OpenAI, or Claude)")
        ...
        has_connection_string = bool(
            cls.BLOB_CONNECTION_STRING and cls.BLOB_CONNECTION_STRING != "UseDevelopmentStorage=true")
        ...
        return {
            ...
            "config": {
                ...
                "blob_storage": "managed_identity" if has_managed_identity_storage else ("connection_string" if has_connection_string else "development")
            }
        }
