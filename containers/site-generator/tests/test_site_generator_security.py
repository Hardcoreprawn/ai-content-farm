...
class TestSiteGeneratorSecurity:
    ...
    def test_sanitize_filename_basic(self, site_generator):
        ...
        assert (
            site_generator._sanitize_filename(None) == "None"
    ...
    async def test_upload_site_archive_path_validation(
        ...
        # Should fail with path outside /tmp (but with .tar.gz extension)
        invalid_path = Path("/etc/test.tar.gz")
        with pytest.raises(
            ValueError, match="Archive path is outside allowed directory structure"
        ):
