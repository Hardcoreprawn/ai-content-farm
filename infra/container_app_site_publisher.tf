...
resource "azurerm_container_app" "site_publisher" {
  ...
  template {
    container {
      ...
      env {
        name  = "HUGO_BASE_URL"
        value = azurerm_storage_account_static_website.main.index_document != null ? "https://${azurerm_storage_account.main.primary_web_endpoint}" : "https://ai-content.example.com"
      }
    }
