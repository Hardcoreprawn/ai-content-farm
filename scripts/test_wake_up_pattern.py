...
3. KEDA scales from 0→1 on any message arrival using azure-queue scaler with managed identity
...
class WakeUpPatternValidator:
    ...
    async def validate_processor_wake_up_logic(self) -> bool:
        ...
        try:
            logger.info(
                "🔍 Validating processor wake-up message handling with Storage Queues..."
            )
            ...
            if not processor_router_path.exists():
                ...
                logger.warning(
                    "⚠️ Storage Queue router not found, checking Service Bus router for migration"
                )

            if not processor_router_path.exists():
                logger.error("✗ No processor router file found")
                return False
    ...
    async def validate_keda_configuration(self) -> bool:
        ...
        try:
            logger.info("🔍 Validating Storage Queue KEDA scaling configuration...")
            ...
            for check_str, description in checks:
                if check_str in tf_content:
                    logger.info(f"✓ Found Storage Queue scaling config: {description}")
                    passed += 1
                else:
                    logger.warning(f"⚠️ Missing Storage Queue config: {description}")

            if passed >= 3:  # At least scaler type, queue length, and scale rule
                logger.info(
                    "✓ KEDA configured for Storage Queue scaling with immediate responsiveness"
                )
                self.test_results["storage_queue_scaling_responsive"] = True
                return True
            else:
                logger.error(
                    "✗ KEDA not configured for Storage Queue responsive scaling"
                )
                return False
    ...
    async def validate_integration_points(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.info("✓ Storage Queue client library available")
                checks.append(True)
            except Exception as e:
                logger.warning(
                    f"⚠️ Storage Queue client library issue (expected during migration): {e}"
                )
                ...
                try:
                    ...
                    logger.info(
                        "✓ Service Bus client library available (fallback during migration)"
                    )
                    checks.append(True)
                except Exception as e2:
                    logger.error(f"✗ No queue client library available: {e2}")
                    checks.append(False)
