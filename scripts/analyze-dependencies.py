...
class DependencyAnalyzer:
    ...
    def generate_summary(self, report: Dict[str, Any]) -> Dict[str, Any]:
        ...
        total_vulns = sum(len(c["vulnerabilities"]) for c in report["containers"].values())
        ...
        return {
            ...
            "containers_with_issues": len([c for c in report["containers"].values() if c["vulnerabilities"] or c["outdated"]]),
            "recommendation": self.get_recommendation(overall_risk, total_vulns, total_outdated)
        }
...
"""
    ...
    def save_report(self, report: Dict[str, Any], filename: str = "dependency-analysis-report.json"):
...
def main():
    ...
    if report['summary']['total_vulnerabilities'] > 0 or report['summary']['total_outdated'] > 0:
