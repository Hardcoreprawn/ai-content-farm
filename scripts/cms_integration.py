...
class HeadlessCMSPublisher:
    ...
    def __init__(self, cms_type: str = "strapi"):
        ...
        self.supported_cms = ["strapi", "contentful",
                              "ghost", "sanity", "netlify-cms"]
    ...
    def prepare_for_strapi(self, manifest_file: str):
        ...
        for post in manifest['posts']:
            ...
            if content.startswith('---'):
                ...
                article = {
                    ...
                    "published_at": frontmatter.get('date', datetime.now().strftime('%Y-%m-%d')),
                    "status": "published",
        ...
        with open(strapi_import, 'w') as f:
            json.dump({
                "version": 2,
                "data": {
                    "api::article.article": strapi_articles
                }
            }, f, indent=2)
...
def demonstrate_cms_integration():
    ...
    manifest_files = list(
        Path("output/markdown").glob("publishing_manifest.json"))
