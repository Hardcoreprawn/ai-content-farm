...
def calculate_function_costs(executions_per_month, avg_duration_seconds, memory_mb):
    ...
    return {
        ...
        'under_free_tier': executions_per_month <= FREE_EXECUTIONS and gb_seconds <= FREE_GB_SECONDS
    }
...
def print_scenario(name, functions, storage, insights, logs):
    ...
    print(f"   Operations: ${storage['read_cost'] + storage['write_cost'] + storage['list_cost']:.2f}")
    ...
    total_cost = (functions['total_cost'] + storage['total_cost'] + 
                  insights['cost'] + logs['cost'])
...
def main():
    ...
    cost_mvp = print_scenario("MVP/Current Plan", functions_mvp, storage_mvp, insights_mvp, logs_mvp)
    
    # Scenario 2: Moderate Growth (5x)
    functions_mod = calculate_function_costs(
        executions_per_month=54_000,
        avg_duration_seconds=15,
        memory_mb=512
    )
    ...
    cost_mod = print_scenario("Moderate Growth (5x)", functions_mod, storage_mod, insights_mod, logs_mod)
    ...
    cost_heavy = print_scenario("Heavy Usage", functions_heavy, storage_heavy, insights_heavy, logs_heavy)
