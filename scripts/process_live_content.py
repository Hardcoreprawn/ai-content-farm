...
def process_live_content():
    ...
    print(f"✅ Created {len(enriched_items)} sample articles with enrichment data")
    ...
    pipeline_output = {
        ...
        "ranked_items": sorted(
            enriched_items, key=lambda x: x["final_score"], reverse=True
        ),
        "metadata": {
...
def create_cms_publishing_manifest(generated_files: dict, output_file: str):
    ...
    manifest = {
        "content_batch": {
            ...
            "total_posts": len(
                [k for k in generated_files.keys() if k.startswith("post_")]
            ),
    ...
    for key, file_path in generated_files.items():
        if key.startswith("post_"):
            rel_path = file_path.replace("/workspaces/ai-content-farm/", "")
            manifest["posts"].append({"id": key, "file": rel_path, "status": "ready"})

    # Save manifest
    manifest_file = (
        "/workspaces/ai-content-farm/output/markdown/publishing_manifest.json"
    )
...
def main():
    ...
    if generated_files:
        ...
        manifest_file = create_cms_publishing_manifest(generated_files, output_file)

        print(f"\n📚 Content Summary:")
        print(
            f'   🔄 Pipeline Output: {output_file.replace("/workspaces/ai-content-farm/", "")}'
        )
        print(f"   📝 Markdown Files: {len(generated_files)} files in output/markdown/")
        print(
