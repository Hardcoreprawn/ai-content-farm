		...
		if "content-collector" in _for_container or _sys.flags.interactive:
			for _m in ("collector", "main", "enricher", "processor"):
				...
				if _path:
					...
					import sys as _sys3
					_sys3.stderr.write(f"[conftest] resolved module {_m} -> (not loaded)\n")
			...
			try:
				...
				if _col_mod:
					...
					if _fetch:
						import sys as _sys4
						_sys4.stderr.write(f"[conftest] collector.fetch_from_subreddit -> id={id(_fetch)} repr={getattr(_fetch, '__name__', repr(_fetch))} module={getattr(_fetch, '__module__', None)}\n")
			except Exception:
				pass
	except Exception:
