...
async def test_article_delivery():
    ...
    try:
        ...
        if jekyll_result:
            print(f"  ✅ Jekyll: Generated {jekyll_result['files_generated']} files")
        ...
        if hugo_result:
            print(f"  ✅ Hugo: Generated {hugo_result['files_generated']} files")
        ...
        blob_url = blob_client.upload_json(
            BlobContainers.PROCESSED_CONTENT, test_blob_name, ranked_content_blob
        )
        ...
        print(f"  ✅ Hugo markdown generation: {hugo_result['files_generated']} files")
