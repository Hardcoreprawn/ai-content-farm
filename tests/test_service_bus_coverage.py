...
class MockTestRouter(ServiceBusRouterBase):
...
class TestServiceBusRouterCoverage:
    ...
    async def test_service_bus_status_retrieval(self, mock_config):
        ...
        # Create mock Service Bus client with config
        mock_client_config = MagicMock()
        mock_client_config.namespace = "test-namespace"

        mock_client = AsyncMock(spec=ServiceBusClient)
        mock_client.config = mock_client_config
